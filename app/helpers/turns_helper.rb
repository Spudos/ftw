module TurnsHelper
  ...
  def perform_completed_upgrades(item)
    ...
    Messages.create(action_id: item.action_id, week: item.week, club: item.club, var1: "Your upgrade to the #{club_full[item.var1.gsub("capacity", "name")]} was completed, the new capacity of this stand is #{club_full[item.var1]}")
  end
end
