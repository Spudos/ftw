class Turn::UpgradeAdmin
  ...
  def perform_completed_upgrades(item, week)
    ...
    if item.var1.start_with?('staff')
      ...
      Message.create(action_id: item.action_id, week: week, club_id: item.club_id, var1: "Your upgrade to the #{item.var1} was completed, the new value is #{club_full[item.var1]}")
      ...
      Message.create(action_id: item.action_id, week: week, club_id: item.club_id, var1: "Your upgrade to the #{club_full[item.var1.gsub("capacity", "name")]} was completed, the new value is #{club_full[item.var1]}")
    end
  end
end
