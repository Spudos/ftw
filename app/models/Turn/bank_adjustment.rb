  ...
  def bank_adjustment
    ...
    if reason == 'coach'
      Message.create(action_id:,
                     week:,
                     club_id:,
                     var1: "Your bank account was charged with #{amount} due to starting an upgrade to #{dept}",
                     var2: 'dec-upgrades', var3: amount.to_i)
      ...
      Message.create(action_id:,
                     week:,
                     club_id:,
                     var1: "Your bank account was charged with #{amount} due to starting an upgrade to #{reason}",
                     var2: 'dec-upgrades', var3: amount.to_i)
      ...
      Message.create(action_id:,
                     week:,
                     club_id:,
                     var1: "You paid an amount to #{dept} to thank him for his contribution to the club.  He now feels more loyal and is more likely to stick with the team in difficult times.  You bank was charged with #{amount}",
                     var2: 'dec-bonuses', var3: amount.to_i)
      ...
      Message.create(action_id:,
                     week:,
                     club_id:,
                     var1: "Your bank account was charged with #{amount} due to starting an upgrade to #{club_full[reason.gsub("capacity", "name")]}",
                     var2: 'dec-upgrades', var3: amount.to_i)
    end
  end
end
