...
RSpec.describe Transfer::TransferUpdates, type: :model do
  describe '#call: listed_player_bids' do
    let(:week) { 1 }
    it 'single listed player transfer bids actioned' do
      create(:transfer, player_id: 1, sell_club: 1, buy_club: 3, bid: 10_000_000)
      ...
      Transfer::TransferUpdates.new(week).call

      expect(Player.first.club_id).to eq(3)
      expect(Player.first.tl).to eq(6)
      expect(Player.first.contract).to eq(24)
      expect(Club.find(1).bank_bal).to eq(10_000_000)
      expect(Club.find(3).bank_bal).to eq(-10_000_000)
    end
    it 'multiple listed player transfer bids actioned' do
      create(:transfer, player_id: 1, sell_club: 1, buy_club: 3, bid: 10_000_000)
      create(:transfer, player_id: 1, sell_club: 1, buy_club: 4, bid: 15_000_000)
      create(:transfer, player_id: 2, sell_club: 2, buy_club: 3, bid: 45_000_000)
      create(:transfer, player_id: 2, sell_club: 2, buy_club: 4, bid: 35_000_000)
      ...
      expect(Club.find(1).bank_bal).to eq(15_000_000)
      expect(Club.find(2).bank_bal).to eq(45_000_000)
      expect(Club.find(3).bank_bal).to eq(-45_000_000)
      expect(Club.find(4).bank_bal).to eq(-15_000_000)
    end
  end
  ...
  describe '#call: deal between two clubs' do
    ...
    it 'valid conditions so transfer completed' do
      create(:transfer, player_id: 1, sell_club: 1, buy_club: 2, bid: 10_000_000, status: 'deal')
      create(:transfer, player_id: 1, sell_club: 1, buy_club: 2, bid: 10_000_000, status: 'deal')

      create(:club, id: 1, bank_bal: 0)
      create(:club, id: 2, bank_bal: 0)

      create(:player, id: 1, club_id: 1, contract: 10, value: 500_000)
      ...
      expect(Player.first.contract).to eq(24)
      expect(Club.find(1).bank_bal).to eq(10_000_000)
      expect(Club.find(2).bank_bal).to eq(-10_000_000)
    end

    it 'fails due to different amounts recorded' do
      create(:transfer, player_id: 2, sell_club: 1, buy_club: 2, bid: 10_000_000, status: 'deal')
      create(:transfer, player_id: 2, sell_club: 1, buy_club: 2, bid: 15_000_000, status: 'deal')
    ...
    it 'fails as the agreed amount is lower than the player value' do
      ...
      create(:player, id: 1, club_id: 1, contract: 10, value: 20_000_000)
